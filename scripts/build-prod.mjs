 ...
 * pionniersdetouraine.fr → basePath vide.
 * (Script Node plutôt que `VAR= next build` pour rester compatible Windows.)
 *
 * Post-traitement : les CSS (charte-hero.css, homepage.css) codent les polices
 * et fonds en dur avec le préfixe /homepagepnr/ : Next ne réécrit pas les url()
 * des feuilles de style. On les ramène à la racine ici.
 */
...
for (const f of readdirSync(cssDir)) {
  ...
  const apres = avant.replaceAll('/homepagepnr/assets/', '/assets/');
  if (apres !== avant) {
